...
def generate_report(results: dict[str, object], output_dir: str | Path) -> Path:
    """Render a technical Markdown report and write it to the output directory."""
    out = Path(output_dir)
...
def _render(results: dict[str, object]) -> str:
    ...
    table = "\n".join(
        "| {strategy} | {attack_successes}/{attack_total} | {asr:.0%} | {intercept_rate:.0%} | "
        "{control_block_rate:.0%} | {avg_guard_latency_ms:.3f} |".format(**row)
        for row in rows
    )

    detail_lines: list[str] = []
    layered = next(r for r in rows if r["strategy"] == "layered")
    for run in layered["attack_runs"]:
        intercept = run["first_intercept_turn"] or "none"
        final_risk = run["turns"][-1]["risk"]["cumulative_risk"]
        detail_lines.append(
            f"- `{run['id']}` ({run['category']}): "
            f"first intercept turn={intercept}, final accumulated risk={final_risk:.2f}."
        )
    attack_detail = "\n".join(detail_lines)

    return f"""# AegisFlow: A Layered Defense Framework for Sequential Escalation Attacks on Instruction-Following LLMs
